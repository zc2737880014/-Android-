...
//        values.put(TodoNote.COLUMN_STATE, State.TODO.intValue);
        //由于State类的重写需要更改put内参数
        values.put(TodoNote.COLUMN_STATE,State.Undo);
        values.put(TodoNote.COLUMN_DATE, System.currentTimeMillis());
//        values.put(TodoNote.COLUMN_PRIORITY, priority.intValue);
        //由于Priority类重写需要更爱put内参数
        values.put(TodoNote.COLUMN_PRIORITY,priority.getValue());
...
//                return Priority.Low;
//        }
          //由于重写了Priority类，getSelectedPriority（）需要重写
          switch (radioGroup.getCheckedRadioButtonId()){
              case R.id.btn_high:
                  return (new Priority(Priority.valueHigh));
              case R.id.btn_medium:
                  return (new Priority(Priority.valueMedium));
              case R.id.btn_low:
                  return (new Priority(Priority.valueLow));
              default:
                  return (new Priority(Priority.valueLow));
          }
    }
