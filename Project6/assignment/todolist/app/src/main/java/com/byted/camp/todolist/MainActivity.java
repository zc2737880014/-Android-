...
//重写note.setState()和note.setPriority()
                note.setState(new State(intState));
                note.setPriority(new Priority(intPriority));
...
//由于重写后State.value为private项需要调用函数得到
        values.put(TodoNote.COLUMN_STATE,note.getState().getValue());
