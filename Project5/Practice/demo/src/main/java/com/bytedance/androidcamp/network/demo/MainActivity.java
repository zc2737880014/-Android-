...
public class MainActivity extends AppCompatActivity {
    ...
    public void testRetrofitSync(View view) throws Exception {
        ...
        new AsyncTask<Object, Integer,Cat>(){

            @Override
            protected Cat doInBackground(Object... objects) {
                Call<List<Cat>> call = getCatService().randomCat(1);
                try {
                    return (call.execute().body().get(0));
                } catch (IOException e) {
                    e.printStackTrace();
                    return null;
                }
            ...
            @Override
            protected void onPostExecute(Cat c) {
                super.onPostExecute(c);
                if(!c.getUrl().isEmpty())
                    tvOut.setText(c.getUrl());
                else
                    tvOut.setText("ERROR!");
            }
        }.execute();
